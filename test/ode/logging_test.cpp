/// The tests of the logging-related utility functions.
/// \file
/// \author Antti Kivi
/// \date 8 April 2018
...
TEST(ode_logging, create_logger)
{
  auto logger_1 = ode::create_logger(
      "test_case_logger_ode_1", "[%H.%M:%S] %v", spdlog::level::info);
...
TEST(ode_logging, trace)
{
  ASSERT_NO_THROW(ode::logging::trace(
      test_log, "This is a test {}-level message", "trace"));
...
TEST(ode_logging, debug)
{
  ASSERT_NO_THROW(ode::logging::debug(
      test_log, "This is a test {}-level message", "debug"));
...
{
  ASSERT_NO_THROW(
      ode::logging::info(test_log, "This is a test {}-level message", "info"));
...
TEST(ode_logging, warn)
{
  ASSERT_NO_THROW(ode::logging::warn(
      test_log, "This is a test {}-level message", "warning"));
...
TEST(ode_logging, critical)
{
  ASSERT_NO_THROW(ode::logging::critical(
      test_log, "This is a test {}-level message", "critical"));
