...
#ifndef ANTHEM_OPTIONAL_H
#define ANTHEM_OPTIONAL_H
...
// 20.5.8, class bad_optional_access
    ...
    public:
      explicit bad_optional_access(const string& what_arg) : logic_error{
          what_arg} {
      }
    ...
    template <class T> union storage_t {
      ...
      template <class... Args> constexpr storage_t(Args&& ... args)
    ...
    template <class T> struct optional_base {
      ...
      template <class U,
                class... Args, TR2_OPTIONAL_REQUIRES(is_constructible<T,
    ...
    template <class T> using OptionalBase = typename std::conditional<
...
# endif
      ...
      void initialize(std::initializer_list<U> il,
                      Args&& ... args) noexcept(noexcept(T(il,
                                                           std::forward<Args>(
                                                               args)...))) {
        assert(!OptionalBase<T>::init_);
    ...
    public:
      ...
      template <class U> auto operator=(U&& v) -> typename enable_if<is_same<
          typename decay<U>::type,
          T>::value, optional&>::type {
        if (initialized()) {
...
# endif
    ...
    public:
      ...
      template <typename U>
      auto operator=(U&& rhs) noexcept -> typename enable_if<is_same<typename decay<
          U>::type, optional<T&>>::value, optional&>::type {
        ref = rhs.ref;
        return *this;
      }
