...
def find_rev_by_timestamp(timestamp, repo_name, refspec):
    base_args = ["git", "log", "-1", "--format=%H",
                 '--before=' + timestamp]
    # On repos with regular batch-automerges from anthem-ci -- namely clang,
    # llvm and lldb -- prefer the most-recent change _made by anthem-ci_
    # before the timestamp, falling back to most-recent in general if there
    # is none by anthem-ci.
    if repo_name in ["llvm", "clang", "lldb"]:
        rev = shell.capture(base_args +
                            ['--author', 'anthem-ci', refspec]).strip()
        if rev:
            return rev
...
def update_single_repository(args):
    ...
    repo_path = os.path.join(ANTHEM_SOURCE_ROOT, repo_name)
...
def get_timestamp_to_match(args):
    if not args.match_timestamp:
        return None
    with shell.pushd(os.path.join(ANTHEM_SOURCE_ROOT, "anthem"),
    ...
    with shell.pushd(ANTHEM_SOURCE_ROOT, dry_run=False, echo=False):
        ...
        if skip_history:
            ...
            shell.run(['git', 'clone', '--recursive', remote,
                       repo_name], echo=True)
        if scheme_name:
            src_path = os.path.join(ANTHEM_SOURCE_ROOT, repo_name, ".git")
            shell.run(['git', '--git-dir', src_path, '--work-tree',
                       os.path.join(ANTHEM_SOURCE_ROOT, repo_name),
                       'checkout', repo_branch], echo=False)
    with shell.pushd(os.path.join(ANTHEM_SOURCE_ROOT, repo_name),
                     dry_run=False, echo=False):
        shell.run(["git", "submodule", "update", "--recursive"],
                  echo=False)


def obtain_all_additional_anthem_sources(args, config, with_ssh, scheme_name,
    ...
    return shell.run_parallel(obtain_additional_anthem_sources, pool_args,
                              args.n_processes)
...
By default, updates your checkouts of Unsung Anthem.""")
    ...
    parser.add_argument("--clone",
                        help="Obtain sources for Unsung Anthem and related "
                             "projects",
                        action="store_true")
    ...
    parser.add_argument("--github-comment",
                        help='Check out related pull requests referenced in '
                             'the given free-form GitHub-style comment.',
                        metavar='GITHUB-COMMENT',
                        dest='github_comment')

    parser.add_argument('--dump-hashes',
                        action='store_true',
                        help='Dump the git hashes of all repositories being '
                             'tracked')

    parser.add_argument('--dump-hashes-config',
                        help='Dump the git hashes of all repositories '
                             'packaged into update-checkout-config.json',
    ...
    parser.add_argument("--match-timestamp",
                        help='Check out adjacent repositories to match '
                             'timestamp of current anthem checkout.',
                        action='store_true')
    ...
    cross_repos_pr = {}
    if github_comment:
        regex_pr = r'(venturesomestone/[-a-zA-Z0-9_]+/pull/\d+|' \
                   r'venturesomestone/[-a-zA-Z0-9_]+#\d+)'
    ...
    if clone or clone_with_ssh:
        ...
        clone_results = obtain_all_additional_anthem_sources(args, config,
                                                             clone_with_ssh,
                                                             scheme,
                                                             skip_history,
                                                             skip_repo_list)
